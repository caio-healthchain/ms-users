-- Lazarus ms-users — DDL incremental/idempotente seguro
-- Data: 2026-05-28
...
-- Guardrails:
--   1. Não executa DROP, TRUNCATE, DELETE ou UPDATE.
--   2. Não usa Prisma db push/reset.
--   3. Cria tabelas se ausentes.
--   4. Adiciona colunas faltantes com ADD COLUMN IF NOT EXISTS.
--   5. Cria índices com CREATE INDEX IF NOT EXISTS.
--   6. Constraints FK são adicionadas apenas se ainda não existirem.
-- Observação importante:
--   Se já existirem tabelas antigas com colunas camelCase, este script NÃO renomeia colunas
--   para evitar risco de quebra/perda. Ele adiciona as colunas snake_case esperadas pelo Prisma atual.
...
-- ============================================================
-- 2. Colunas aditivas para ambientes onde as tabelas já existem
-- ============================================================
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_hospital_profiles_user_id_fkey') THEN
        ALTER TABLE "user_hospital_profiles"
          ADD CONSTRAINT "user_hospital_profiles_user_id_fkey"
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_hospital_profiles_hospital_id_fkey') THEN
        ALTER TABLE "user_hospital_profiles"
          ADD CONSTRAINT "user_hospital_profiles_hospital_id_fkey"
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_hospital_profiles_profile_id_fkey') THEN
        ALTER TABLE "user_hospital_profiles"
          ADD CONSTRAINT "user_hospital_profiles_profile_id_fkey"
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'auth_audit_logs_user_id_fkey') THEN
